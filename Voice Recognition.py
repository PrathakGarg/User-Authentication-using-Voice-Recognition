...
def generate_codebook(data, size_codebook, epsilon=0.00001):
    ...
    while len(codebook) < size_codebook:
        codebook, codebook_abs_weights, codebook_rel_weights, avg_dist = split_codebook(data, codebook, epsilon,
                                                                                        avg_dist)
...
def matching(data, codebook):
    closest_c_list = [None] * len(
        data)  # _size_data # list that contains the nearest codevector for each input data vector
...
Choice = input(
    ...
    "to Check matching with other voices by inputting a new voice\n Enter 4 to print file\n Enter 5 to delete a voice "
    "from Database\n Enter 6 to check shape of file\n Enter 7 to Store a new voice to database live(Through "
